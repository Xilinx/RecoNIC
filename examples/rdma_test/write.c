...
int main(int argc, char *argv[])
{
  ...
  uint32_t cidb_buffer_size = (1 << 21);
...
if(client) {
    ...
    gen_init_wqe_cmd(&init_wqe_cmd, cmd_size, 0, 0, sq_pidb_addr, wrid, RNIC_OP_WRITE, 1, (uint64_t) device_buffer->buffer, payload_size, (uint64_t) write_offset_client, R_KEY, 0, 0, 0, 0, 0, sq_addr_msb, sq_addr_lsb);

    print_init_wqe_cmd(&init_wqe_cmd);

    issue_init_wqe_cmd((void *)rdma_dev->axil_ctl, RN_CLR_CTL_CMD, &init_wqe_cmd);
    qpid_wqecount = qpid;
    qpid_wqecount = (qpid_wqecount << 16) | 0x0001;
    ...
    sent_tmp = (uint32_t* ) device_buffer->buffer;

    for (uint32_t i = 0; i < payload_size>>2; i++) {
        fprintf(stderr, "Info: sent data: recv[%d]=%d\n", i, sent_tmp[i]);
    }*/
  }
