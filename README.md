...
To meet the explosive growth of data and scale-out workloads/applications, today's data center architecture has evolved significantly and consists of sea of nodes connected via the network. Each node in the architecture typically contains a host with multi-core CPUs and accelerators in the form of ASIC, FPGA, and/or GPU. It connects the network through a smart network interface controller (SmartNIC) targeting high-speed network typically above 40Gbps. Due to its intrinsic heterogeneity, today's data center architecture provides an ideal environment for distributed applications. In traditional distributed applications with computation offloaded on accelerators, data received from the network is first stored in the host’s memory and then copied to the memory of accelerators for compute acceleration via the PCIe bus. After computation, results are usually copied again back to the host’s memory. The overhead of several data copies makes this CPU-centric solution significantly lower down system performance and incur high latency.

To address the challenges, we propose RecoNIC, an RDMA-enabled smartNIC platform with compute acceleration, to reduce overhead of data copies and bring data as near as possible to computation. It consists of a hardware shell and software stack. The RecoNIC hardware shell provides basic NIC functionalities, an RDMA engine and two compute logic boxes for lookside and streaming applications. Users can leverage the proposed solution to access either device or host memory resided in remote peers via RDMA. Moreover, users can design their accelerators either in RTL or HLS in the RecoNIC's compute logic box and process data from the network without multiple copies in the traditional CPU-centric solutions.

## RecoNIC System Overview

<img src="https://github.com/Xilinx/RecoNIC/blob/main/doc/image/RecoNIC.png" width=70% height=70%>

The above figure shows the hardware shell and software stack of RecoNIC. The hardware shell consists of a basic NIC module (including MAC subsystem and QDMA subsystem), a packet classification module, an RDMA engine, a compute logic module and a few auxiliary modules (including system/memory crossbars and an arbiter). 

RDMA traffic is processed by the RDMA engine and payload data from the network can be stored either in host memory or device memory. User defined accelerators can be implemented in the Compute Logic and directly process network data in the device memory.
